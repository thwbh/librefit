...
<!--  <div class="flex flex-col items-center self-center w-32">
      <p>
        {total}
      </p>
      <p>
...
-->  
  <figure class="progress-radial relative overflow-hidden w-32" data-testid="progress-radial">
    <svg viewBox="0 0 {width} {height}" class="rounded-full">
      <g stroke-width={stroke}>
        <path
          d={arc(width / 2, height / 2, outerRadius, 0, 359)}
          class="progress-radial-track fill-transparent stroke-primary/50"
      ...
      <g stroke-width={stroke}>
        <path
          ...
          class="progress-radial-track fill-transparent stroke-secondary/50"
          stroke-linecap="round"
        />
        ...
        {#if calorieTarget}
          <path
            ...
            class="progress-radial-track fill-transparent stroke-secondary"
          />
      ...
      >
        {#if calorieTarget}
          {@const leftover = limit - total}
          {#if leftover < 0}
            <tspan class="text-amount">{Math.abs(leftover)}</tspan>
            <tspan class="text-info opacity-60">over</tspan>
          {:else}
            <tspan class="text-amount">{leftover}</tspan>
            <tspan class="text-info opacity-60">left</tspan>
          {/if}
        {/if}
      </text>
