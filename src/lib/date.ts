...
export function convertDateStrToDisplayTimeStr(str: string, displayFormat?: string | undefined) {
	const date = parseStringAsDate(str, displayFormat);

	return format(date, display_time_format.valueOf());
}

export function parseStringAsDateTime(date: string, time: string, dateFormat?: string | undefined) {
	if (!dateFormat) {
		dateFormat = default_date_time_format;
	}

	return parse(`${date} ${time}`, dateFormat.valueOf(), new Date());
}
