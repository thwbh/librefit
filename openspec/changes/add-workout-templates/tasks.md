...
- [ ] 2.1 Template CRUD repository + commands (name/description, ordered exercises, target reps/weight, notes) [WO-037]
- [ ] 2.2 Clone command: deep-copy a predefined template to a new `is_predefined = FALSE` template; source unchanged; predefined templates not editable [WO-038]
- [ ] 2.3 Swap-exercise command rebuilding dense `sequence` within a transaction [WO-039]
- [ ] 2.4 Start-from-template: create a session prefilled with the template's exercises in order, subject to single-active-session [WO-040]
- [ ] 2.5 Extend the exercise delete-guard to also refuse exercises referenced by a template (builds on WO-032)
...
- [ ] 5.1 Rust integration tests for build/clone/swap/start-from, each citing its WO id via `scenario!` [WO-037..WO-040]
