...
- **Decision:** Keep a single `exercise` table rather than splitting seeded and user rows. Harden it:
  ...
  - Add `verified BOOLEAN NOT NULL DEFAULT 0` (seeded rows seeded as `1`); `created_at TEXT` (for graceful decay); make `category` nullable (a name-only "Ghost" has none yet).
