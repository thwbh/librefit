...
**What stays relational (NOT in the payload):** muscles (seeded `muscle` lookup + `exercise_muscle` join, `role` primary/secondary) — they're queried ("all chest exercises"), so they stay normalized. The flexible column is reserved for genuinely type-varying _measurements_, not for anything we filter/join on. The `muscle` lookup's `shortvalue` is a `svelte-body-highlighter` slug (`chest`, `upper-back`, `gluteal`, `abs`, …) — one shared muscle vocabulary across the dataset and the completion-screen heat map, so there is no frontend muscle-name mapping and a future exercise-curation UI (edit/add exercises) can offer the same tags end to end.
