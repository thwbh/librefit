...
The home dashboard SHALL compose the feature surfaces it hosts. When no workout session is active, it SHALL display the calorie card, then the weight card, then the workout surface, in that order. The workout surface SHALL show summary cards for today's completed workouts when any exist; otherwise it SHALL show the Start Workout module. The internal behavior of each card is owned by its feature spec; this capability owns only their arrangement and the dashboard's adaptive states.
...
#### Scenario: [DH-011] Completed workouts shown instead of Start Workout
...
- **AND** the Start Workout module is not shown
