...
pub struct NewCalorieTracker {
    ...
    pub added: String,
    #[validate(range(min = 1, max = 10000, message = "Calorie amount must be between 1 and 10,000"))]
    pub amount: i32,
...
pub struct NewWeightTracker {
    #[validate(custom(function = "validate_date_format"))]
    pub added: String,
    #[validate(range(min = 30.0, max = 330.0, message = "Weight must be between 30 and 330 kg"))]
    pub amount: f32,
}
...
pub struct NewCalorieTarget {
    ...
    #[validate(custom(function = "validate_date_format"))]
    pub end_date: String,
    #[validate(range(min = 1, max = 10000, message = "Maximum calories must be between 1 and 10,000"))]
    pub maximum_calories: i32,
    #[validate(custom(function = "validate_date_format"))]
    pub start_date: String,
    #[validate(range(min = 1, max = 10000, message = "Target calories must be between 1 and 10,000"))]
    pub target_calories: i32,
}
...
pub struct NewWeightTarget {
    ...
    #[validate(custom(function = "validate_date_format"))]
    pub end_date: String,
    #[validate(range(min = 30.0, max = 300.0, message = "Initial weight must be between 30 and 300 kg"))]
    pub initial_weight: f32,
    #[validate(custom(function = "validate_date_format"))]
    pub start_date: String,
    #[validate(range(min = 30.0, max = 300.0, message = "Target weight must be between 30 and 300 kg"))]
    pub target_weight: f32,
}
...
fn validate_date_format(date_str: &str) -> Result<(), ValidationError> {
    match NaiveDate::parse_from_str(date_str, "%Y-%m-%d") {
        Ok(_) => Ok(()),
        Err(_) => Err(ValidationError::new("Invalid date format. Expected YYYY-MM-DD")),
    }
}
...
fn validate_calorie_target(target: &NewCalorieTarget) -> Result<(), ValidationError> {
    ...
    if target.target_calories > target.maximum_calories {
        return Err(ValidationError::new("Target calories cannot exceed maximum calories"));
    }
...
fn validate_weight_target(target: &NewWeightTarget) -> Result<(), ValidationError> {
    ...
    let weight_change_ratio = (target.target_weight - target.initial_weight).abs() / target.initial_weight;
    if weight_change_ratio > 0.5 {
        return Err(ValidationError::new("Weight change cannot exceed 50% of initial weight"));
    }
