...
pub fn daily_dashboard(pool: State<DbPool>, date_str: String) -> Result<Dashboard, String> {
    ...
    match parse_result {
        Ok(date) => {
            ...
            let calorie_target = find_last_calorie_target(&mut conn)
            ...
            match (
                ...
                (Ok(calorie_target_start_date), Ok(calorie_target_end_date)) => {
                    ...
                    let calories_today_vec = find_calorie_tracker_by_date(&mut conn, &date_str)
                        .unwrap_or_else(|_| vec![]);
                    let calories_week_vec =
                        find_calorie_tracker_by_date_range(&mut conn, &week_start_str, &date_str)
                            .unwrap_or_else(|_| vec![]);
