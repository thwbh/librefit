...
pub fn get_weight_targets() -> Result<Vec<WeightTarget>, String> {
    with_db_connection(weight::get_weight_targets).map_err(handle_error)
}

/// Retrieve last weight target
#[command]
pub fn get_last_weight_target() -> Result<WeightTarget, String> {
    with_db_connection(weight::get_latest_weight_target).map_err(handle_error)
}
...
pub fn update_weight_target(
    ...
    with_db_connection(|conn| weight::update_weight_target(conn, target_id, updated_target))
        .map_err(handle_error)
}
...
pub fn create_weight_tracker_entry(new_entry: NewWeightTracker) -> Result<WeightTracker, String> {
    ...
    with_db_connection(|conn| weight::create_weight_tracker_entry(conn, &new_entry))
        .map_err(handle_error)
}
...
pub fn delete_weight_tracker_entry(tracker_id: i32) -> Result<usize, String> {
    with_db_connection(|conn| weight::delete_weight_tracker_entry(conn, tracker_id))
        .map_err(handle_error)
}
